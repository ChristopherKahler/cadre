...
def test_denied_tool_returns_structured_payload_not_prose(
    ...
    # A member holding the key still cannot decide a gate. The tool surface
    # enforces that by omission: requesting and viewing are exposed, approving
    # is not wired at all, so there is nothing for a key-holder to reach.
    request_gate(conn, "chrisai", {
    ...
    assert hasattr(tools_mod, "firm_request_gate")
    assert hasattr(tools_mod, "firm_view_gate")
    for name in ("firm_approve_gate", "firm_decide_gate", "firm_resolve_gate"):
        assert not hasattr(tools_mod, name)
