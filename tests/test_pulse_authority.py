...
from firm.pulse.spawn import SpawnResult, _run_env
...
def test_run_env_stamps_the_member_id() -> None:
    env = _run_env("MEM-007")
    assert env is not None
    assert env["CADRE_MEMBER_ID"] == "MEM-007"
    # Inherits the rest of the parent env — the child still needs PATH etc.
    assert "PATH" in env


def test_run_env_none_inherits_unchanged() -> None:
    # An unidentified spawn must not fabricate an identity.
    assert _run_env(None) is None
