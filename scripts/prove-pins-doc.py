...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as t:
        ...
        # The PRE-FIX script, found by history rather than named by sha and no
        # longer read from origin/main. It was read from origin/main while the
        # floor was unmerged; the floor is now ON main, so origin/main returns
        # the FIXED script and this control silently stopped being a control.
        # A control that quietly grades the thing it is the control for is
        # worse than no control.
        intro = subprocess.run(
            ["git", "-C", str(W), "log", "--format=%H", "--reverse",
             "-S", "RC_NOTHING_SCANNED", "--", "scripts/check-action-pins.py"],
            capture_output=True, text=True, timeout=60)
        shas = [l for l in intro.stdout.split() if l]
        if not shas:
            sys.exit("REFUSING: could not find the commit that introduced the "
                     "floor, so the pre-fix script cannot be located and this "
                     "control would prove nothing.")
        old = subprocess.run(
            ["git", "-C", str(W), "show",
             shas[0] + "^:scripts/check-action-pins.py"],
            capture_output=True, text=True, timeout=60)
        if old.returncode != 0 or "def offenders" not in old.stdout:
            sys.exit("REFUSING: could not read the pre-fix script from "
                     f"{shas[0][:9]}^; the control would prove nothing.")
        if "RC_NOTHING_SCANNED" in old.stdout:
            sys.exit(f"REFUSING: {shas[0][:9]}^ already carries the floor, so "
                     "it is not a pre-fix script and this control would be "
                     "grading the fix against itself.")
