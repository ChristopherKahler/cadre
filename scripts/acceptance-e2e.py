    ...
    try:
        ...
        def end_base_section() -> int:
            ...
            b.assert_section(base_section_start, BASE_SECTION_ROWS,
                             "base as the engine")
            # No `is not None` guard: `base_home` is now built unconditionally
            # at the top of this section, above the probe, so the directory
            # always exists by the time any path reaches here and a check that
            # cannot be false would read as a live guard while guarding nothing.
            shutil.rmtree(base_home, ignore_errors=True)
